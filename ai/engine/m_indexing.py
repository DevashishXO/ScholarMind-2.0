...
def add_papers_to_index(collection: chromadb.Collection, papers: list):
    ...
    # Debugging: Verify norms of embeddings
    import numpy as np
    norms = [np.linalg.norm(e) for e in embeddings]
    print(f"Embedding norms before adding to collection: {norms}")  # Should all be close to 1
...
def fetch_arxiv_and_index(query: str = "machine learning", max_results: int = 10):
    ...
    arxiv_client = arxiv.Client(page_size=2000, delay_seconds=5, num_retries=5)
    search = arxiv.Search(
        query=query,
    ...
    papers = []
    try:
        log(f"Attempting single-page fetch with page_size=2000 and 5s delay...")
...
if __name__ == "__main__":
    fetch_arxiv_and_index(query="machine learning in healthcare", max_results=100)
